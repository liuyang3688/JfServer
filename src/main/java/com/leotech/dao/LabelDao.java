...
@Repository
public class LabelDao extends ExHibernateDaoSupport {
	@Autowired
	...
	{
		return (List<Label>)getHibernateTemplate().find("from " + Label.class.getName());
	}
			...
			jdbcTemplate.query(strSql, new RowCallbackHandler(){
				public void processRow(ResultSet result) throws SQLException {
					...
					try{
						...
						label.put("pos_x", result.getDouble("pos_x"));
						label.put("pos_y", result.getDouble("pos_y"));
						label.put("pos_z", result.getDouble("pos_z"));
