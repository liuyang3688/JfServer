...
@Repository
public class MeshDao extends ExHibernateDaoSupport {
	@Autowired
	private JdbcTemplate jdbcTemplate;

	public List<Mesh> getAll()
	{
		return (List<Mesh>)getHibernateTemplate().find("from " + Mesh.class.getName());
	}
		...
		try {
			...
			jdbcTemplate.query(strSql, new RowCallbackHandler() {
				public void processRow(ResultSet result) throws SQLException {
					...
					mesh.put("pos_x", result.getDouble("pos_x"));
					mesh.put("pos_y", result.getDouble("pos_y"));
					mesh.put("pos_z", result.getDouble("pos_z"));
