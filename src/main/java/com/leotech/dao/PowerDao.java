...
@Repository
public class PowerDao extends ExHibernateDaoSupport {
	@Autowired
	private JdbcTemplate jdbcTemplate;
	private static Map<String, PowerInfo > powerInfoMap = null;
	public List<Power> getAll()
	{
		return (List<Power>)getHibernateTemplate().find("from " + Power.class.getName());
	}
	...
	public void fetchPowerCount(){
		try {
			if (powerInfoMap == null) {
				powerInfoMap = new HashMap<String, PowerInfo>();
			} else {
				powerInfoMap.clear();
			}
			powerInfoMap.clear();
			// From统计
			String sqlFilter = " where power.fromDevice = device.code and device.typeid = device_type.id and power.isShow=1";
			...
			//To统计
			sqlFilter = " where power.toDevice = device.code and device.typeid = device_type.id and power.isShow=1";
