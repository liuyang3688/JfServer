...
public class PowerComThread implements Runnable {
	...
	public void readConfig(){
		...
		try {
			InputStream in = AirCondComThread.class.getResourceAsStream("/config/comm.properties");
			prop.load(in);
			conf.setCommId(prop.getProperty("power.commId"));
			conf.setBaudRate(Integer.parseInt(prop.getProperty("power.baudRate", "2400")));
			conf.setDataBit(Integer.parseInt(prop.getProperty("power.dataBit", "8")));
			conf.setStopBit(Integer.parseInt(prop.getProperty("power.stopBit", "1")));
			conf.setParity(Integer.parseInt(prop.getProperty("power.parity", "0")));
			conf.setDTR(Boolean.parseBoolean(prop.getProperty("power.dtr", "true")));
			conf.setRTS(Boolean.parseBoolean(prop.getProperty("power.rts", "true")));
			in.close();
			...
			try {
				...
				while (running) {
					...
					while (availableBytes > 0) {
						...
						// 从写入位置开始读取
						int realReadCount = inputStream.read(buffer, writePos, 100);
						...
						if (!needReq){
							if (writePos - readPos >= needReadNum) {
									...
									if (nCalcCrc == nResCrc) {
										...
										if (reqParam.code == 1 || reqParam.code == 2) {
											...
											for (int pos=0; pos < DCount; ++pos ) {
												...
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 0), (int)(data>>0 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 1), (int)(data>>1 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 2), (int)(data>>2 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 3), (int)(data>>3 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 4), (int)(data>>4 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 5), (int)(data>>5 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 6), (int)(data>>6 & 0x01));
												RtService.setRtData_YX(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos*8 + 7), (int)(data>>7 & 0x01));
											}
											...
											for (int pos=0; pos < DCount; pos+=2 ) {
												...
												RtService.setRtData_YC(new Triple(Constant.BJLX_POWER, devId, reqParam.startAdd + pos/2), (double)(val * 0.1));
											}
										} else {
											bParsed = false;
										}
...
//			prop.load(in);
//			conf.setCommId(prop.getProperty("power.commId"));
//			conf.setBaudRate(Integer.parseInt(prop.getProperty("power.baudRate", "9600")));
//			conf.setDataBit(Integer.parseInt(prop.getProperty("power.dataBit", "8")));
//			conf.setStopBit(Integer.parseInt(prop.getProperty("power.stopBit", "1")));
//			conf.setParity(Integer.parseInt(prop.getProperty("power.parity", "0")));
//			conf.setDTR(Boolean.parseBoolean(prop.getProperty("power.dtr", "false")));
//			conf.setRTS(Boolean.parseBoolean(prop.getProperty("power.rts", "false")));
//			in.close();
//		} catch (FileNotFoundException e) {
//			System.out.println("File Not Found: comm.properties");
//		} catch (Exception e) {
...
//		}
//	}
//	public InputStream openCom() throws NoSuchPortException,
//			PortInUseException,
//			IOException,
//			UnsupportedCommOperationException{
//		CommPortIdentifier com = CommPortIdentifier.getPortIdentifier(conf.getCommId());
//		serialCom = (SerialPort) com.open("Main", 1000);
//		serialCom.setSerialPortParams(
//				conf.getBaudRate(),
//				conf.getDataBit(),
...
//		System.out.println(conf.getCommId() + "：端口已监听");
//		// 循环读取
//		return serialCom.getInputStream();
//	}
//	public void run() {
//		readConfig();
//		byte[] buffer = new byte[1024];
...
//		while(running) {
//			try {
//				InputStream inputStream = openCom();
//				Arrays.fill(buffer, (byte)0);
//				int availableBytes = 0;
//				int writePos = 0;
//				int readPos = 0;
