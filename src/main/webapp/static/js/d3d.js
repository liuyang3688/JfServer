...
//     this.updateFromDB();
// };
D3DLib.prototype.home = function(){
    window.location.href = g_base + "/index";
}
//Load Event From DB
D3DLib.prototype.loadEvtFromDB = function () {
	$.ajax({
		url:g_base+'/3d/get_all_event',
		dataType:'json',
		data: null,
...
D3DLib.prototype.parseBtnData = function(data) {
    if (data != null && data.length > 0) {
        ...
        $['each'](data, function (i, obj) {
            $('#menutoolbar')['append'](' <img style="width: 80px;" src="' + GCONFIG['pic_path'] + obj['img'] + '" title="' + obj['title'] + '" id="' + obj['name'] + '" />');
    ...
    if (intersectObjects['length'] > 0) {
        ...
        if (SEL_MESH instanceof THREE.Mesh &&
            ...
            if (SEL_MESH !== D3DOBJ.LAST_MESH ) {
                ...
                if (SEL_MESH['name'].indexOf('dev_') === 0 && GCONFIG['vparam']) {
                    D3DOBJ.showDeviceInfo(SEL_MESH);
                }
            }
            // 当前物体非网线和机柜
            if (SEL_MESH['name'].indexOf('dev_') === 0 ||
                SEL_MESH['name'].indexOf('col_') === 0 ||
                SEL_MESH['name'].indexOf('row_') === 0) {
...
//////////////////////////////////////////////////
D3DLib.prototype.vparam = function(){
    GCONFIG['vparam'] = !GCONFIG['vparam'];
};
...
D3DLib.prototype.showCable = function(prefixCode) {
    D3DOBJ.deleteCable();
    if (prefixCode === null || prefixCode ==='' || prefixCode === undefined ) {
        prefixCode = '1A1';
    }
    $.ajax({
        url: g_base+'/3d/get_some_cable',
        dataType: 'json',
        ...
        success: function (datas) {
            ...
            for(let i=0; i<datas.length; ++i){
                let data = datas[i];
                let fmEthName = data['fmCode'];
                let toEthName = data['toCode'];
                ...
                let mesh = D3DOBJ.createPath(connName, connType, fmPt, toPt, params);
...
D3DLib.prototype.deleteCable = function() {
    for (let i=0; i<GCONFIG['cableList'].length; ++i){
        let pathName = GCONFIG['cableList'][i];
        D3DOBJ.delObject(pathName);
    }
}
D3DLib.prototype.toggleCable = function(prefixCode){
    // 关闭电缆供应
    // if (GCONFIG['power']) {
    //     D3DOBJ.power();
    // }
    if (!GCONFIG['conn']) {
        D3DOBJ.showCable(prefixCode);
    } else {
        D3DOBJ.deleteCable(prefixCode);
    };
...
// D3DLib.prototype.space_ratio = function() {
//     if(GCONFIG['load_ratio']) {
//         D3DOBJ.load_ratio();
//     }
//     if (!GCONFIG['space_ratio']){
...
//             config['width'] = 240;
//             config['height'] = 280;
//             config['pos_x'] = cab['position']['x'];
//             config['pos_y'] = GCONFIG['cab_inner_height'] + config['height'] / 8;
//             config['pos_z'] = cab['position']['z'];
//             let msgArr = [];
...
//         D3DOBJ['cabInfoSpriteList']=[];
//     }
//     GCONFIG['space_ratio'] = !GCONFIG['space_ratio'];
// };
D3DLib.prototype.space_ratio = function() {
    if(GCONFIG['load_ratio']) {
        D3DOBJ.load_ratio();
    ...
    if (!GCONFIG['space_ratio']){
        $.ajax({
            url: g_base+'/3d/get_all_stats_load',
            dataType: 'json',
            data: null,
            async: false,
            success: function (datas) {
                for(let i=0; i<datas.length; ++i) {
                    let data = datas[i];
                    let msgArr = [];
                    msgArr.push('机柜编码:'+data['name']);
                    msgArr.push('所属分区:'+ data['areaName']);
                    msgArr.push('所属系统:'+ data['systemName']);
                    msgArr.push('总U数:'+data['totalU']);
                    msgArr.push('已用U数:'+data['usedU']);
                    msgArr.push('剩余U数:'+data['remainU']);
                    msgArr.push('最大连续U:'+data['maxConsU']);
                    msgArr.push('空间使用率:'+data['spaceRatio'].toFixed(2)+"%");

                    // 根据名字获取对象
                    let cabName = 'cab_' + data['name'].substr(1);
                    let cab = D3DOBJ.findObject(cabName);

                    // 配置参数
                    let config = {};
                    config['name'] = 'sprite_'+data['name'];
                    config['font'] = '18px FangSong';
                    config['background'] = getColorByRatio(data['spaceRatio']);
                    config['textColor'] = 'white';
                    config['width'] = 240;
                    config['height'] = 280;
                    config['pos_x'] = cab['position']['x'];
...
D3DLib.prototype.load_ratio = function() {
    if(GCONFIG['space_ratio']) {
        D3DOBJ.space_ratio();
    }
    if (!GCONFIG['load_ratio']){
        $.ajax({
            url: g_base+'/3d/get_all_stats_space',
            dataType: 'json',
            data: null,
            async: false,
            success: function (datas) {
                for(let i=0; i<datas.length; ++i) {
                    ...
                    msgArr.push('所属分区:'+ data['areaName']);
                    msgArr.push('所属系统:'+ data['systemName']);
                    msgArr.push('总容量:'+data['totalCap']);
                    msgArr.push('已用容量:'+data['usedCap']);
                    msgArr.push('剩余容量:'+data['remainCap']);
                    msgArr.push('负载使用率:'+data['loadRatio'].toFixed(2)+"%");
                    ...
                    config['name'] = 'sprite_'+data['name'];
                    config['font'] = '18px FangSong';
                    config['background'] = getColorByRatio(data['loadRatio']);
                    config['textColor'] = 'white';
                    config['width'] = 240;
                    config['height'] = 280;
                    config['pos_x'] = cab['position']['x'];
    ...
    GCONFIG['load_ratio'] = !GCONFIG['load_ratio'];
};
...
D3DLib.prototype.help = function(){
    $.messager.show({'title':'使用帮助','height':400,'msg': '' +
        '<B>通用编码解释</B><BR>' +
        '例如: <font style="color:red">1A101020304F</font><BR>' +
        '1(机房编码) A1(机柜编码) 01(设备起始U位) 02(设备占用U数) 03(网口行位置) 04(网口列位置) F(前向) <BR><BR>' +
        '1. 如何选择某排机柜<BR>' +
        '选择当前排两侧机柜的行标签，右键单击即可。<BR>'+
        '2. 如何选择单个机柜<BR>' +
        '选择当前机柜顶上的列标签，右键单击即可<BR>' +
        '3. 如何查看机柜内设备<BR>' +
        '左键双击机柜的前门即可<BR>'+
        '4. 如何查看单台设备信息<BR>'+
        '左键单击设备即可<BR>'+
        ...
        '点击菜单-供电电缆即可<BR>'+
        '7. 查看空调风向<BR>'+
        '点击菜单-空调风向，即可查看当前机房的空调风走向<BR>'+
        '8. 查看机柜利用率</div><BR>'+
        '点击菜单-空间利用率，即可查看所有记过使用情况。<BR>'+
        '9. 查看动环数据<BR>'+
        '点击菜单-动环数据，即可查看动环设备实时数据刷新情况。<BR>'+
        '10. 选择推荐机柜<BR>'+
        '点击菜单-推荐机柜，即可按照对话框提示进行新加设备的机柜选择。<BR>'
        });
    // if (!GCONFIG['help']){
    //     $('#help').show();
    // } else {
...
D3DLib.prototype.createPower = function(pathName, pathType, fmPt, toPt, params) {
    ...
    // let g=Math.floor(Math.random()*256);
    // let b=Math.floor(Math.random()*256);
    let imgurl = g_base+'/static/pic/UV_Red.png';
    switch (params['type']) {
        case 1: imgurl = g_base+'/static/pic/UV_Green.png'; break;
        case 2: imgurl = g_base+'/static/pic/UV_Red.png'; break;
        case 3: imgurl = g_base+'/static/pic/UV_Grid_Sm.jpg'; break;
    }
    let pathConfig = {
        name: pathName,
        type: 'beeline',
...
D3DLib.prototype.parseDeviceData = function(datas){
    for(let i=0; i<datas.length; ++i){
        ...
        data['pos_x'] = 0;
        data['pos_y'] = GCONFIG['u_height']*(startU-1 + (useU*1.0/2)) - GCONFIG['cab_inner_height']/2;
        data['pos_z'] = 0;
        data['rot_y'] = 180;
...
D3DLib.prototype.drawLabel = function(data){
    ...
    mesh.name = data['name'];
    mesh.uuid = data['id'];
    mesh.position.y = data["pos_y"];
    mesh.position.x = data["pos_x"];
    mesh.position.z = data["pos_z"];
...
D3DLib.prototype.cloneObj = function(cloneData){
    ...
    cloneObj['position']['x'] += cloneData['pos_x'] || 0;
    cloneObj['position']['y'] += cloneData['pos_y'] || 0;
    cloneObj['position']['z'] += cloneData['pos_z'] || 0;
...
D3DLib.prototype.createPlane = function(meshData, side_config){
    ...
    mesh['position'].set(meshData['pos_x'],meshData['pos_y'],meshData['pos_z']);
    return mesh;
};
...
D3DLib.prototype.addTunnel = function (config, pointsArr) {
    ...
    if (config['type'] == null || typeof(config['type']) === 'undefined' || config['type'] === 'curve') {
        ...
        CustomCurve['prototype']['getPoint'] = function (pointIndex) {
            ...
            let vindex = parseInt( pointIndex / per);
            if (1 === pointIndex) {
                let point = new THREE.Vector3(pointsArr[vindex]['x'], pointsArr[vindex]['y'],pointsArr[vindex]['z']);
                return point;
            };
            let nVertor3 = new THREE.Vector3();
...
D3DLib.prototype.makeDynamicTextSprite = function(name, config){
    ...
    sprite['position']['x'] = config.hasOwnProperty('pos_x') ? config['pos_x'] : 0;
    sprite['position']['y'] = config.hasOwnProperty('pos_y') ? config['pos_y'] : 0;
    sprite['position']['z'] = config.hasOwnProperty('pos_z') ? config['pos_z'] : 0;
...
D3DLib.prototype.addPlane = function(planeConfig, sideConfig) {
	planeConfig['pos_x'] = planeConfig.position.x;
	planeConfig['pos_y'] = planeConfig.position.y;
	planeConfig['pos_z'] = planeConfig.position.z;
...
D3DLib.prototype.hideCabinet = function(obj) {
    ...
    if(!obj.isShowCabinet) {
        if(GCONFIG['conn']){
            var prefixCode = '1' + obj.name.substr(4);
            D3DOBJ.showCable(prefixCode);
            return;
        } else if (GCONFIG['power']) {
            return;
        } else if (GCONFIG['space_ratio']) {
            return;
        }
    }
    DelBoxHelper();
...
D3DLib.prototype.hideRowCabinet = function(obj) {
    if(!obj.isShowCabinet) {
        if(GCONFIG['conn'] || GCONFIG['power'] || GCONFIG['space_ratio'] || GCONFIG['load_ratio']) {
            return;
        }
    }
    DelBoxHelper();
...
D3DLib.prototype.showCableInfo = function(obj) {
    if (!obj['userData']['show_cable_info']){
        ...
        let url = g_base + '/3d/get_cable_info?name=' + obj['userData']['name'];
        g_propwin_show(url, D3DOBJ['mousePos']);
        obj['userData']['show_power_info'] = true;

        // $.ajax({
        //     type:'post',
        //     url:g_base+'/3d/get_eth_info',
        //     dataType:'json',
...
let DEV_INFO_ID = 0;
function g_propwin_close(){
    $('#propwin').window('close');
}
function g_propwin_show_data(data, pos){
    $('#propwin').css({
        left: pos['x'],
        top: pos['y']
    });
...
function g_propwin_show(url, pos) {
    $('#propwin').css({
        left: pos['x'],
        top: pos['y']
    });
    $('#propwin').window('open');
    $('#propwin-pg').propertygrid({url:url});
}
D3DLib.prototype.showDeviceInfo = function(obj) {
    if (!obj['userData']['show_dev_info']){
        ...
        let url = g_base+'/3d/get_device_info?devCode=' + devCode;
        g_propwin_show(url, D3DOBJ['mousePos']);
        //g_show_device();
        // $.ajax({
        //     type:'post',
        //     url:g_base+'/3d/get_device_info',
        //     dataType:'json',
...
D3DLib.prototype.showPowerInfo = function(obj) {
    if (!obj['userData']['show_power_info']){
        ...
        let code = powerName.substr(6);
        var arr = code.split('-');
        if (arr.length === 2) {
            var data = {
                'total': 3,
                'rows': [
                    {'name': '电缆名称', 'value': powerName},
                    {'name': '供电A端', 'value': arr[0]},
                    {'name': '供电B端', 'value': arr[1]},
                ]
            }
            g_propwin_show_data(data, D3DOBJ['mousePos']);
            obj['userData']['show_power_info'] = true;
        }
    }
};
